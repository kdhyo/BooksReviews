...
직접 코드를 작성하면서 느낀 바로는 정말 시간도 단축되고, 코드도 깔끔하고, 안전하고..  
...
---
### 🤔 오늘 읽은 소감은? 더오르는 생각을 가볍게 적어보세요.
지금까지는 내가 직접 쓰레드를 여러 개를 만들어서 사용하는 것보다,
spring, spring boot 에서 자동으로 멀티 스레드를 지원해주는 것들만 사용해왔기 때문에
쓰레드에 대해 깊게 이해하지 못하고 있었다.

그 의미처럼 난 멀티 쓰레드 환경에서는 synchronized 예약어를 통해서  
락을 걸면 부하가 생기는 상황이 있더라도 다 해결될 것 이라는 잘못된 생각을 가지고 있었다.  
이번 파트를 통해서 다양한 예외 상황들을 생각하고, 테스트 케이스를 만들고,  
직접 여러 os환경이나 여러 변수들을 생각하면서 짜야된다는 것을 배울 수 있었다.  

조만간 직접 여기 나와있는 예시들을 하나하나 테스트를 진행해보아야 할 것 같다.
...
> 지난 수십여 년 동안 쌓아온 경험에서 얻은 교훈이라면,  
프로그래밍은 과학보다 공예에 가깝다는 사실이다.  
깨끗한 코드를 짜려면 먼저 지저분한 코드를 짠 "뒤에 정리해야 한다"는 의미다.

> 작문을 할 때 먼저 초안을 쓰라고 하셨다.  
먼저 1차 초안을 쓰고, 그 초안을 고쳐 2차 초안을 만들고,  
계속 고쳐 최종안을 만들라 말씀하셨다.  
...
---
### 🤔 오늘 읽은 소감은? 더오르는 생각을 가볍게 적어보세요.
처음부터 완벽하게 코드를 짜는 사람은 정말 없을 것이다.  
처음부터 복잡하게 생각하고 확장을 생각해서 여러 기능들을 넣을 필요도 없다.  
...
이번 파트에서 나는 위에 말을 프로그래머로 살면서 평생 새기고 싶다.  
처음부터 완벽하게란 불가능하지만,  
새로운 기능을 추가하다가 리팩토링할 부분을 캐치하고, 더 늦기 전에 코드를 리팩토링 하면서  
기능들을 하나씩 쌓아 올리는 것.. 꼭 필요한 부분이라 생각된다.

중요한건, 현재 내 코드가 유지보수 하기가 좋은 코드인지 보는 눈과  
판단할 줄아는 상황판단 능력, 바로 기능 추가를 멈추고 리팩토링을 진행할 수 있는 진행력.  
꾸준히 기르고 싶다.
...
- 깔끔한 작품을 내놓으려면 단계적으로 개선해야 한다고 가르치려 애쓰셨다.
- 현재 내 코드 상황을 파악하고, 개선점을 찾는 눈을 기르고 싶다.
...
### 😀 책에서 기억하고 싶은 내용을 써보세요.
> 프로그램을 망치는 가장 좋은 방법 중 하나는 개선이라는 이름 아래  
구조를 크게 뒤집는 행위다.  
...
> 기존 코드 끝에 ArgumentMarshaler 클래스의 골격을 추가했다.  
당연히 해당 코드 추가는 아무 문제도 일으키지 않는다.

> 다음으로 나는 코드를 최소로 건드리는, 가장 단순한 변경을 가했다.  
구체적으로 Boolean 인수를 저장하는 HashMap에서 Boolean 인수 유형을  
ArgumentMarshaler 유형으로 바꿨다.

> 그러면, 코드 일부가 깨지게 되고 깨진 부분을 재빠르게 고쳤다.

> 불행히도, 사소한 변경이었으나 몇몇 테스트 케이스가 실패하기 시작했다.

> 다음 변경으로 넘어가기 전에 나는 재빨리 이것부터 고쳐야 했다.  
실제로 그다지 어렵지 않았다.

> 이번에도 앞서 구현과 마찬가지로 한 번에 하나씩 고치면서 테스트를 계속 돌렸다.  
테스트 케이스가 하나라도 실패하면 다음 변경으로 넘어가기 전에 오류를 수정했다.

> 지금쯤이면 내 의도를 눈치챘으리라.  
일단 각 인수 유형을 처리하는 코드를 모두 ArgumentMarshaler 클래스에 넣고나서  
ArgumentMarshaler 파생 클래스를 만들어 코드를 분리할 작정이었다.  
그러면, 프로그램 구조를 조금씩 변경하는 동안에도  
시스템의 정상 동작을 유지하기 쉬워지기 때문이다.

</br>

---
### 🤔 오늘 읽은 소감은? 더오르는 생각을 가볍게 적어보세요.
예제를 보여주면서 작가가 직접 리팩토링 하는 순서를 이해하기 쉽게 알려주었다.  

이번 리팩토링에서 가장 중요한 건 "테스트 케이스"다.  
모든 리팩토링에 있어서 사소한 것 하나 수정할 때마다 "테스트 케이스"가 정상적으로  
작동하는지를 확인하면서 넘어가는 방식으로 리팩토링을 진행하고 있다.  
하지만, 만약에 테스트 케이스가 정확하지 않거나 예외상황이 빠져있게 된다면..  
나의 "테스트 케이스"를 의지하지 못하게 될 것이다.  
이는 이 리팩토링 방식에서 가장 중요한 부분이라 생각된다.  

무조건, 테스트 케이스가 제대로 잘 작성되어 있어야 된다.  
그래야, 믿고 리팩토링을 마음편하게 할 수 있을 것이다.  

그럼 책의 예제처럼 "테스트 케이스"가 제대로 작성되어 있다는 가정하에  
책에서 리팩토링을 진행한 방식을 정리해보면 정말 사소한 것 부터 진행한다.  

1. 코드에 문제가 없게 변경할 새로운 클래스를 추가한다.
2. 코드를 가장 최소로 건드리는 변경을 진행한다. (예시로는 Boolean -> ArgumentMarshaler)
3. 테스트 케이스를 돌린다.
4. 테스트 케이스가 실패한 오류 부분을 찾아서 고친다.
5. 2번으로 돌아가서 2~4를 반복한다.

한 번에 크게 리팩토링을 진행하는 것이 아니라,  
정말 최소로 변경 후에 테스트 케이스를 계속 확인하면서  
점진적으로 변경해 나아간 걸 알 수 있었다.  

이것이 TDD 사이클..!  
여기서 내가 미처 생각지 못했던 부분은 변경 예정인 새로 만들 클래스도  
간단하게 만들어 나아가면서 리팩토링을 했다는 것이다.  
난 지금까지 새로 만들 클래스를 완벽하게 짜기 위해서 노력했었지만,  
그건 말처럼 쉽지도, 되지도 않고, 시간만 많이 잡아먹었던 기억이 난다..
</br>

---
### 👀 소감 3줄 요약
- 가장 중요한건 믿고 리팩토링할 수 있는 "테스트 케이스"를 작성해 두는 것.
- 기존 코드를 싹 엎는 것이 아니라 사소한 것 하나씩 변경해 나아가자.
- 하나 변경할 때마다 테스트 케이스 확인은 필수!
...
> setBooleanArg 메소드는 iterator가 필요없다.  
그런데도 iterator를 인수로 넘긴 이유가 무엇이냐고?  
setIntArg와 setStringArg에서 필요하기 때문이다!  
또한 setBooleanArg, setIntArg, setStringArg 함수 모드를  
ArgumentMarshaler에 set 함수가 있다면 직접 호출할 수 있다.  
따라서 set 함수를 만들 시점이다.  

```
public abstrat void set(Iterator<String> currentArgument)
...
---
### 🤔 오늘 읽은 소감은? 더오르는 생각을 가볍게 적어보세요.
리팩토링을 진행할 때에도 한 번에 좋은 아이디어가 떠올라서 바로 실천할 수는 없다.  
그렇다면, 이미 처음 짤 때부터 멋진 코드를 짰을테니까..  
그렇기에 리팩터링을 할 때에도 하나의 큰 목표를 잡고 여러 단계들을 거쳐서  
그 목표를 이루는 방식으로 진행해야 된다는 걸 이번 파트를 보면서 새삼 느끼게 되었다.  

나는 인터페이스나 추상클래스를 만들 때 인수를 모든 파생 클래스에서  
사용할 메소드만 만들어야 된다고 생각했다.  
하지만 지금 리팩토링을 하는 것을 보면서 다수에서 인수를 사용하기 위해서  
인수를 넣었지만, 사용하지 않는 메서드가 존재할 수도 있고, 저런 식으로도 짜도  
괜찮다..? 라고 생각을 해볼 수 있는 기회였다.  
저 방식이 무조건 옳다 괜찮다 까지는 잘 모르겠지만, 저런 방식으로도 짤 수 있구나를 배웠다.

</br>

---
### 👀 소감 3줄 요약
- 리팩토링할 때에도 한 번에 변경해야 된다는 생각은 하지말자.
- 파생 메소드에서 인수가 필요 없어도 다른 파생 메소드들에서 필요하다면..?
- 다형성을 통해 3 종류가 하나의 코드로 바뀌는 마법. 멋지다.
...
> 오류메세지를 Args 클래스와 ArgsException 클래스 중 어디서 처리해야 옳을까?  
ArgsException에게 맡겨서는 안 된다고 생각하는 독자라면 새로운 클래스가 필요하다.  
하지만 미리 깔끔하게 만들어진 오류 메세지로 얻는 장점은 무시하기 어렵다.  
...
---
### 🤔 오늘 읽은 소감은? 더오르는 생각을 가볍게 적어보세요.
4일에 걸쳐서 이번 파트를 끝을 냈다.  

직접 하나하나 어떻게 리팩토링을 진행하는지 볼 수 있는 기회여서 인상깊었고,  
완벽하게 짜려는 생각보다 코드를 작성하면서 계속해서 더 나은 방법을 생각하는 것이  
좋은 방법이라 생각이 든다.
</br>
...
### 👀 소감 3줄 요약
- 단순히 돌아가는 코드에 만족하는 프로그래머는 전문가 정신이 부족하다.
- 나쁜 일정, 요구사항은 다시 정하면 된다. 하지만, 나쁜 코드는 썩어 문드러진다.
- 코드는 언제나 최대한 깔끔하고 단순하게 정리하자.
...
테스트 케이스가 모든 행, 모든 if문, 모든 for문을 실행한다는 의미다.  
그래서 나는 모듈이 올바로 동작한다고 자신하게 되었고  
모듈 작성자들의 장인정신을 높이 사게 되었다.
...
> 함수에서 멤버변수와 이름이 똑같은 변수를 사용하는 이유가 무엇일까?  
서로 다른 의미가 아닌가? 이름은 명확하게 붙인다.  
...
---
### 🤔 오늘 읽은 소감은? 더오르는 생각을 가볍게 적어보세요.
이번 파트 제목을 보고, JUnit 관련 테스트 코드에 대해서 설명하는 파트일 줄 알았는데,  
리팩토링을 진행하는 파트라서 조금 당황했다...  
이전파트와 마찬가지로 기존 코드를 가지고, 하나씩 리팩토링 하는 걸 보여주었는데,  
이번에는 처음에도 어느정도 괜찮은 코드라고 느끼는 코드를 수정하는 걸 보여주면서  
책 초반에 나온 보이스카우트 규칙을 다시 상기시켜주어 인상깊었다.  

기존 코드도 유심히 보고, 시간 될 때 하나씩 수정해보는 것도 중요한 경험이겠구나 싶다.
...
---
### 👀 소감 3줄 요약
- 함수에서 멤버변수와 이름이 똑같은 변수를 사용하는 이유가 무엇일까? 서로 다른 의미가 아닌가?
- 메소드명에 인수까지 고려하면서 지으면 가독성이 훨씬 더 좋아진다.
- 리팩터링은 시행착오를 겪는다, 수정한 코드도 망설임없이 수정하자.
...
우리 모두가 편안하게 여겨야 할 활동이다.  
남이 내게 해준다면 감사히 반겨야 할 활동이다.  
이와 같은 비판이 있어야만 발전도 가능하다.  
의사들도 한다. 조종사들도 한다. 법률가들도 한다.  
우리 프로그래머들도 방법을 배워야 한다.

serialVersionUID 변수를 사용할지 말지에 대한 의견
> 이 책을 검토한 검토자 여러 명이 내 의견에 반론을 제기했다.  
그들은 오픈 소스 프레임워크에서는 직렬화 ID를 직접 제어하는 편이 낫다고 주장했다.  
사소한 변경을 가했을 뿐인데 이전 클래스를 복원하지 못하는 사태를 피하기 위해서다.  
타당한 의견이라 생각한다. 하지만 자동 제어를 사용함으로써 발생하는 오류는  
(개발자 입장에서는 불편할지라도) 원인과 결과가 분명하다.  
반면, 클래스 작성자가 깜빡하고 ID를 갱신하지 않아 발생하는 오류는 원인도 결과도 모호하다.  
...
### 🤔 오늘 읽은 소감은? 더오르는 생각을 가볍게 적어보세요.
리팩토링 순서  
1. 처음에 나온 오래된 주석을 간단하게 고치고, 개선했다. (이력제거 등)
2. enum을 모두 독자적인 소스 파일로 옮겼다.
3. 정적 변수(dateFormatSymbols)와 정적 메서드(getMonthNames, isLeapYear, lastDayOfMonth)를  
DateUtil이라는 새 클래스로 옮겼다.
4. 일부 추상 메서드를 DayDate 클래스로 끌어올렸다.
5. Month.make를 Month.fromInt로 변경했다.  
다른 enum도 똑같이 변경했다.  
또한 모든 enum에 toInt() 접근자를 생성하고 index 필드를 private로 정의했다.  
6. plusYears와 plusMonths에 흥미로운 중복이 있었다.  
correctLastDayOfMonth라는 새 메서드를 생성해 중복을 없앴다.  
그래서 세 메서드가 모두 좀 더 명확해졌다.
7. 팔방미인으로 사용하던 숫자 1을 없앴다.  
모두 Month.JANUARY.toInt() 혹은 Day.SUNDAY.toInt()로 적절히 변경했다.  
SpreadsheetDate 코드를 살펴보고 알고리즘을 조금 손봤다.  

리팩토링의 끝은 과연 있을까 싶다.  
분명 처음에도 잘짰다고 감탄했던 코드들을 막상 하나하나 열어보고, 수정하고, 결과를 보니  
더욱 멋진 코드가 나온 걸 보게 되었다.  

이번 파트는 약간 코드리뷰의 대한 생각을 정리해볼 수 있었다.  
아무리 뛰어난 개발자라 하더라도 해당 개발자가 짠 코드를 우리가 분석하고,  
이야기를 나눌 수 있다는 걸 느꼈고 그걸 기분나빠하면 발전이 더디겠구나 생각이 들었다.  

</br>

---
### 👀 소감 3줄 요약
- 전문가 입장에서 내 코드를 검토해주고, 내가 검토해주는 행위는 모두가 편안하게 받아드러야 한다.
- 상수를 사용할 때는 enum을 최대한 활용할 수 있는지 생각해보자.
- 보이스카우트 규칙을 따르자. 체크아웃한 코드보다 더 깨끗하게 체크인 하자.
...
### 주석
1. 부적절한 정보
    - 소스코드관리시스템(svn, git), 버그추적시스템, 이슈추척시스템 등에 저장할 정보는  
    주석으로 적절하지 못하다.  
    일반적으로 작성자, 최종 수정일, SPR(Software Problem Report)번호 등과 같은  
    메타 정보만 주석으로 넣는다. 주석은 코드와 설계에 기술적인 설명을 부연하는 수단이다.
2. 쓸모 없는 주석
    - 오래된 주석, 엉뚱한 주석, 잘못된 주석은 빨리 지우고,  
    쓸모 없어질 주석은 아예 달지 않는 편이 가장 좋다.
3. 중복된 주석
    - 다음과 같이 코드만으로 충분한데 구구절절 설명하는 중복된 주석은 만들지 말자.  
    i++; // i 증가
4. 성의 없는 주석
...
5. 주석 처리된 코드
    ...
    주석으로 처리된 코드는 흉물 그 자체다. 보이면 바로 지워라. 걱정할 필요 없다.  
    소스 코드 관리 시스템이 기억하니까.
...
### 함수
1. 너무 많은 인수
    - 인수는 아예 없으면 가장 좋다. 다음으로 하나, 둘, 셋 차례로 좋다.  
    넷 이상은 그 가치가 아주 의심스러우므로 최댛나 피한다.
2. 출력 인수
    - 인수는 일반적으로 입력으로 간주한다.  
    함수에서 뭔가의 상태를 변경해야 한다면 함수가 속한 객체의 상태를 변경한다.  
    public void appendFooter(StringBuffer report) 보다는  
    report.appendFooter() 가 좋다.
3. 플래그 인수
    - boolean 인수는 함수가 여러 기능을 수행한다는 명백한 증거다. 피하자.
4. 죽은 함수
    - 아무도 호출하지 않는 함수는 즉시 삭제한다. 어차피 코드관리시스템에서 모두 기억한다.

### 일반
1. 한 소스 파일에 여러 언어를 사용한다.
    - 어떤 JSP 파일은 HTML, 자바, 태그 라이브러리 구문, 영어 주석, Javadoc, XML,  
    JavaScript 등을 포함한다. 좋게 말하면 혼란스럽고, 나쁘게 말하면 조잡하다.  
    현실적으로는 여러 언어가 불가피하다. 하지만 각별한 노력을 기울여 소스 파일에서  
    언어 수와 범위를 최대한 줄이도록 애써야 한다.
2. 당연한 동작을 구현하지 않는다.
    - 함수나 클래스는 다른 프로그래머가 당연하게 여길 만한 동작과 기능을 제공해야 한다.  
    당연한 동작을 구현하지 않으면 코드를 읽거나 사용하는 사람이 더 이상 함수 이름만으로  
    함수 기능을 직관적으로 예상하기 어렵게 되고,  
    저자를 신뢰하지 못하므로 코드를 일일이 살펴보게 된다.
3. 경계를 올바로 처리하지 않는다.
    - 부지런함을 대신할 지름길은 없다. 모든 경계 조건, 모든 구석진 곳, 모든 기벽,  
    모든 예외는 우아하고 직관적인 알고리즘을 좌초시킬 암초다.  
    스스로의 직관에 의존하지 마라. 모든 경계 조건을 찾아내고, 모든 경계 조건을  
    테스트하는 테스트 케이스를 작성하라.
4. 안전 절차 무시
    - 컴파일러 경고 일부 혹은 전부를 꺼버리명 빌드가 쉬워질지 모르지만 자칫하면  
    끝없는 디버깅에 시달린다.  
    실패하는 테스트 케이스를 일단 제껴두고 나중으로 미루는 태도는  
    신용카드가 공짜 돈이라는 생각만큼 위험하다.
5. 중복
    - 이 책에 나오는 가장 중요한 규칙 중 하나다.  
    코드에서 중복이 발견될 때마다 추상화할 기회로 간주하라.  
    ...
    switch/case나 if/else 문으로 똑같은 조건을 거듭 확인하는 코드도 중복이다.  
...
7. 기초 클래스가 파생 클래스에 의존한다.
    ...
    기초 클래스와 파생 클래스를 다른 jar로 배포해도 가능하게 설계하자.
8. 과도한 정보
...
9. 죽은 코드
    - 불가능한 조건을 확인하는 if문, throw가 없는 try 문에서 catch 블록은  
    실행되지 않는 코드. 즉, 죽은 코드다. 이러한 코드는 시간이 지나면 악취를 풍긴다.  
    죽은 코드를 발견하면 올바른 행동을 취하길 바란다. 시스템에서 제거하라.
10. 수직 분리
...
11. 일관성 부족
    ...
    다른 함수에서도 일관성 있게 동일한 변수 이름을 사용하라.  
...
15. 선택자(플래그) 인수
    ...
    일반적으로, 인수를 넘겨 동작은 선택하는 대신 새로운 함수를 만드는 편이 좋다.
...
---
### 🤔 오늘 읽은 소감은? 더오르는 생각을 가볍게 적어보세요.
한 달 조금 넘게 클린 코드를 읽으면서 솔직히 초반 내용이 모두 생각나지는 않는다.  
이번 챕터는 그런 우릴 위해서 지금까지 내용을 한 번 리마인드 시켜주는  
아주 중요한 챕터라고 생각된다.  
가벼운 마음으로, 하나하나 그 뜻을 이해하면서 읽어 나아가야겠다.

</br>

---
### 👀 소감 3줄 요약
- 한 달간 긴 시간동안 내용을 정리할 수 있는 챕터
- 주석이든 코드든 사용하지 않는 건 과감하게 지우자. 우리에겐 svn, git이 있다!
- 클래스명, 함수명, 변수명을 올바르게 짓고, 거기에 맞는 일만 하자.
...
17. 잘못 지은 책임
    - 상수, 로직 등 사용하는 곳에 배치를 시키자.  
    어느 곳에 배치해야할지 애매할 때에는 이름을 보고, 맞는 위치에 배치하자.
18. 부적절한 static 함수
    - 일반적으로 static 함수보다 인스턴스 함수가 더 좋다.  
    조금이라도 의심스럽다면 인스턴스 함수로 정의한다.  
    반드시 static 함수로 정의해야겠다면 재정의할 가능성은 없는지 꼼꼼히 따져본다.
19. 서술적 변수
    - 서술적인 변수 이름은 일반적으로는 많을수록 더 좋다.  
    계산을 몇 단계로 나누고 중간 값에 좋은 변수 이름만 붙여도 해독하기 어렵던  
    모듈이 순식간에 읽기 쉬운 모듈로 탈바꿈한다.
20. 이름과 기능이 일치하는 함수
    - Date newDate = date.add(5); 코드를 보자.  
    5일? 5주? 5시간? 어떤걸 더하는 건가?  
    또 date 인스턴스를 변경하는건가? 예전 date을 두고 새로운 Date을 반환하는 함수인가?  
    이름만으로 분명하지 않기에 구현을 살피거나 문서를 뒤적여야 한다면  
    도 좋은 이름으로 바꾸거나 아니면 더 좋은 이름을 붙이기 쉽도록 기능을 정리해야 한다.
21. 알고리즘을 이해하라.
    - 구현이 끝났다고 선언하기 전에 함수가 돌아가는 방식을 확실히 이해하는지 확인하라.  
    테스트 케이스를 모두 통과한다는 사실만으로 부족하다.  
    작성자가 알고리즘이 올바르다는 사실을 알아야 한다.  
    알고리즘이 올바르다는 사실을 확인하고 이해하려면  
    기능이 뻔히 보일 정도로 함수를 깔끔하고 명확하게 재구성하는 방법이 최고다.
22. 논리적 의존성은 물리적으로 드러내라.
    - A라는 클래스에서 B라는 클래스를 사용할 때에  
    B에서 A가 무엇이든 알고 있다고 가정하면 안된다.  
    B에서는 A가 모른다고 생각하여야 하며, A에서 필요하면 B에서 가져와서 써야한다.
23. IF/Else 혹은 Switch/Case 문보다 다형성을 사용하라.
    - 대다수 개발자가 switch 문을 사용하는 이유는 그 상황에서 가장 올바른 선택이기 보다는  
    당장 손쉬운 선택이기 때문이다. 그러므로 switch를 선택하기 전에 다형성을 먼저 고려하라.  
...
24. 표준 표기법을 따르라.
    ...
    팀이 정한 표준은 모든 팀원들이 따라야 한다.  
    실제 괄호를 넣는 위치는 중요하지 않다.  
    모두가 동의한 위치에 넣는다는 사실이 중요하다. 이 사실을 이해할 정도로 팀원들이 성숙해야 한다.
25. 매직 숫자는 명명된 상수로 교체하라.
    - 아마 소프트웨어 개발에서 가장 오래된 규칙 중 하나라 여겨진다.  
    일반적으로 코드에서 숫자를 사용하지 말라는 규칙이다. 숫자는 명명된 상수 뒤로 숨기라는 의미다.  
    "매직 숫자"라는 용어는 단지 숫자만 의미하지 않는다. 의미가 분명하지 않은 토큰을 모두 가르킨다.
26. 정확하라
...
28. 조건을 캡슐화하라.
    - if문과 while문 조건을 분명히 밝히는 함수로 표현하라.
29. 부정 조건은 피하라.
...
31. 숨겨진 시간적인 결합
    ...
    ```
    - 위 코드는 세 함수가 실행되는 순서가 중요하지만, 시간적인 결합을 강제하지 않는다.  
    호출 순서를 잘못 넣어서 오류가 발생해도 막을 도리가 없다. 다음 코드가 더 좋다.  
    ```
    public class MoogDiver {
        Gradient gradient;
        List<Spline> splines;
        public void dive(String reason) {
            ...
            List<Spliner> splines = eticulateSplines(gradient);
...
33. 경계 조건을 캡슐화하라.
    ...
    ```
    - nextLevel 처럼 이런 경계 조건은 변수로 캡슐화하는 편이 좋다.
...
21. 알고리즘을 이해하라.  
22. 논리적 의존성은 물리적으로 드러내라.
위 내용들은 알면서도 잘 실천하지 못하는 부분들이었다.  
다시 내용을 상기시키면서 꼭 실천하도록 노력해보자.  
아래 소감 3줄 요약에 위 내용을 하나씩 적어보았다!

</br>

---
### 👀 소감 3줄 요약
- 메소드 호출을 한 결과들을 이어이어 만드는 것 보다 하나하나 변수로 만드는 게 나중에 읽기 더 좋다.
- 테스트 케이스를 믿기 전에 내가 이 알고리즘을 명확하게 이해했는지를 꼼꼼히 따져보자. 
- 다른 클래스에서 이건 알고 있어야지. 라고 짜기 전에 사용될 클래스에 알려주는 방식을 생각하자.
...
34. 함수는 추상화 수준을 한 단계만 내려가야 한다.
    ...
    ```
    - 뒤섞인 추상화 수준을 멋지게 분리한다.  
    render 함수는 HR 태그만 생성한다. HR 태그 문법은 전혀 상관하지 않는다.  
    HTML 문법은 HtmlTag 모듈이 알아서 처리한다. size의 뜻도 명확히 파악했다.
...
36. 추이적 탐색을 피하라.
    ...
    여러 모듈에서 위와같은 형태를 사용한다면 설계와 아키텍처를 바꿔 B와 C 사이에 Q를 넣기가 쉽지 않다.  
    a.getB().getC()를 모두 찾아 a.getB().getQ().getC()로 바꿔야 하니까.  
    너무 많은 모듈이 아키텍처를 너무 많이안다. 그래서 아키텍처가 굳어진다.  
    다음과 같은 간단한 코드로 충분해야 한다.  
    myCollaborator.doSomething();  

### 자바
1. 긴 import 목록을 피하고 와읻르 카드를 사용하라.
    - 명시적 import 문은 강한 의존성을 생성하지만 와일드카드는 그렇지 않다.  
    명시적으로 클래스를 import하면 반드시 그 클래스가 존재해야 한다.  
    하지만 와일드 카드로 패키지를 지정하면 특정 클래스가 존재할 필요는 없다.  
    import 문은 패키지를 단순히 검색 경로에 추가하므로 진정한 의존성이 생기지 않는다.  
    그러므로 모듈 간에 결합성이 낮아진다.
2. 상수는 상속하지 않는다.
    - 인터페이스에 상수를 넣고, 인터페이스를 구현하고, 해당 구현 클래스를 상속하는 행위는 끔찍하다.  
    대신, static import를 사용하라.  
    import static PayrollConstants.*;
3. 상수 대 Enum
    - public static final int 라는 옛날 기교를 더 이상 사용할 필요 없다. Enum을 사용하자!  
...
1. 서술적인 이름을 사용하라.
    ...
    isStrike() 함수가 하는 일을 충분히 미뤄 짐작할 수 있다.
2. 적정한 추상화 수준에서 이름을 선택하라.
    - 리팩토링 전  
    ```
    ...
    ```
    - 기존 코드는 연결 대상의 이름을 전화번호로 제한적이었다.  
    이제는 전화번호는 물론이고 다른 연결 방식에도 사용 가능하다.
3. 가능하다면 표준 명명법을 사용하라.
    - 자바에서 객체를 문자열로 변환하는 함수는 toString이라는 이름을 많이 쓴다.  
    이런 이름은 관례를 따르는 편이 좋다.  
    흔히 팀마다 특정 프로젝트에 적용할 표준을 나름대로 고안한다.  
    이를 프로젝트의 유비쿼터스 언어라 부르며, 코드는 이 언어에 속하는 용어를 열심히 써야한다.  
    간단히 말해, 프로젝트에 유효한 의미가 담긴 이름을 많이 사용할수록 독자가 코드를 이해하기 쉬워진다.
4. 명확한 이름
    - 함수나 변수나 이름이 아주 길다는 단점을 서술성이 충분히 메꾼다.
5. 긴 범위는 긴 이름을 사용하라
    - 이름 길이는 범위 길이에 비례한다. 범위가 작으면 아주 짧은 이름을 사용해도 괜찮다.  
    하지만, 범위가 길어지면 긴 이름을 사용한다.
6. 인코딩을 피하라.
    - m_, f, List와 같은 이름은 중복된 정보이며 독자만 혼란하게 만든다.  
...
7. 이름으로 부수 효과를 설명하라.
    ...
    ```
    - 위 함수는 단순히 "oos"만 가져오지 않는다. 기존에 없으면 생성한다.  
    그러므로, creatOrReturnOos 라는 이름이 더 좋다.  
...
---
### 🤔 오늘 읽은 소감은? 더오르는 생각을 가볍게 적어보세요.
지금까지 약 한 달 반 동안 클린코드를 읽으면서 실무에 적용할 수 있는 여러가지를 알게 되었다.  
물론, 한 번에 다 익히지는 못하였지만 하나하나 천천히 휴리스틱 목록에 나오는 것들을  
내 것으로 흡수하고 싶다.
...
- 추상화 수준은 한 단계만 내려가자.
- 느린 테스트는 실행하지 않게 된다.
- 위에 말한 규칙만 따른다고 깨끗한 코드를 얻지는 못한다. 전문가 정신과 장인 정신은 가치에서 나온다.

</br>
</br>

#노마드클럽 #북클럽 #노개북 #클완스 #클린코드완독스터디